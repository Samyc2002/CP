...
bool saarsh(vector<pair<ll, ll>> v, pair<ll, ll> val) {
    foi(i, v.size()) {
...
int main() {
    ...
    map<ll, ll> first;
    map<ll, ll> second;
    vector<pair<ll, ll>> weed;
    foi(i, m) {
        first.insert(pair<ll, ll>(i+1, 0));
    }
    foi(i, n) {
        second.insert(pair<ll, ll>(i+1, 0));
    }
    foi(i, k) {
        ll a, b;
        cin >> a >> b;
        first[a]++;
        second[b]++;
        weed.PB(make_pair(a-1, b-1));
    }
    ll max_row = 0;
    ll max_row_pos = 0;
    foi(i, m) {
        if(first[i+1] > max_row) {
            max_row = first[i+1];
            max_row_pos = i+1;
        }
    }
    ...
    sort(all(weed));
    ll sub = 0;
    if(saarsh(weed, make_pair(max_row_pos-1, max_col_pos-1))) {
        sub++;
    }
    cout << max_row+max_col-sub << endl;
    return 0;
}