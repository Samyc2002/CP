...
int test1(int full, int partial, int unsolved, int x, int y) {
    if(full>=x) {
        return 1;
    }
    else {
...
int test2(int full, int partial, int unsolved, int x, int y) {
    if((full==(x-1)) && (partial>=y)) {
        return 1;
    }
    else {
        return 0;
...
int main() {
    ...
    foi(i, t) {
        ...
        foi(j, n) {
            cin >> attempts;
            int f = full(attempts, m);
            int p = partial(attempts, m);
            int u = unsolved(attempts, m);
            int t1 = test1(f, p, u, x, y);
            int t2 = test2(f, p, u, x, y);
            if(t1+t2!=0) {
                pf[j] = 1;
            }
        }
