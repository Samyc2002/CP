...
int main() {
    ...
    while(t--) {
        int n, k, f;
        cin >> n >> k >> f;
        vi start(n), end(n), duration(n);
        ll invigilation_time = 0;
        foi(i, n) {
            cin >> start[i] >> end[i];
            duration[i] = end[i]-start[i];
            invigilation_time += duration[i];
        }
        int overlap = 0;
        foi(i, n) {
            foi(j, n) {
                if(start[j]<end[i] && i!=j) {
                    overlap += (end[i]-start[j]);
                }
            }
        }
        if((invigilation_time-overlap)<=(f-k)) {
            cout << "YES" << endl;
        }
        else {
            cout << "NO" << endl;
