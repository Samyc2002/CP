...
int main() {
    ...
    while(t--) {
        ...
        while(!ticket.empty()) {
            int index = ticket.front().first;
            int value = ticket.front().second;
            ticket.pop();
            if(occupied.find(value) == occupied.end()) {
                occupied.insert(pii(value, index));
            } else {
                value++;
                ticket.push({ index, value });
            }
        }
        vi seat(n);
        map<int, int>::iterator itr;
        for(itr = occupied.begin(); itr != occupied.end(); itr++) {
            seat[itr->second-1] = itr->first;
        }
        foi(i, n) {
            cout << seat[i] << " ";
        }
