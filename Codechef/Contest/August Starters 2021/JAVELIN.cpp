...
int main() {
    ...
    while(t--) {
        int n, m, x;
        cin >> n >> m >> x;
        vector<pii> distance(n);
        foi(i, n) {
            int q;
            cin >> q;
            distance[i] = make_pair(q, i+1);
        }
        sort(all(distance), sortDecs);
        vi qualified;
        foi(i, n) {
            if(distance[i].first >= m || qualified.size() < x) {
                qualified.PB(distance[i].second);
            }
        }
        sort(all(qualified));
        cout << qualified.size() << " ";
        foi(i, qualified.size()) {
            cout << qualified[i] << " ";
        }
        cout << endl;
    }
