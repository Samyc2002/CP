...
int invertedSum(int sum, string charges, int pos) {
    if(pos == 0) {
        if(charges[pos] == charges[pos+1]) {
            sum++;
        }
        else {
            sum--;
...
int main() {
    ...
    while(t--) {
        ...
        foi(i, k) {
            charge[pos[i]] = invert(charge[pos[i]]);
            sum = invertedSum(sum, charge, pos[i]);
            cout << sum << endl;
        }
    }
    return 0;
