...
int main() {
    ...
    string path;
    cin >> path;
    vector<bool> isLilyPresent(n, false);
    vi lilies;
    
    foi(i, path.length()) {
        if(path[i] == '1') {
            isLilyPresent[i] = true;
            lilies.PB(i);
        }
    }
    ...
    ll max_gap=0;
    foi(i, lilies.size()-1) {
        if(lilies[i+1]-lilies[i] > max_gap) {
            max_gap = lilies[i+1]-lilies[i];
        }
    }
    ...
    if(max_gap <= d) {
        ...
        ll steps=0;
        while(n-i-1 >d) {
            if(isLilyPresent[i+d]) {
                i+=d;
                steps++;
            }
            else {
                ll j=i+d;
                while(!isLilyPresent[j]) {
                    j--;
                }
                i=j;
                steps++;
